...
//float3 ks = float3(0.25f, 0.25f, 0.25f);//1,1,1でテカる
	//float3 ks = float3(0, 0, 0);//1,1,1でテカる
    float3 ks = float3(1, 1, 1); //1,1,1でテカる
    ...
    float3 directionalDiffuse = CalcLambertDiffuse(N, L, directionalLightData.color.rgb, kd);
	//float3 directionalDiffuse = ClacHalfLambert(N, L, directionalLightData.color.rgb, kd);//ハーフランバート版
    float3 directionalSpecular = CalcPhongSpecular(N, L, directionalLightData.color.rgb, E, shiness, ks);
